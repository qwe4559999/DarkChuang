...
async def analyze_spectrum(
    ...
    try:
        ...
        # 进行光谱分析
        analysis_result = await image_service.analyze_spectrum_image(
...
async def batch_analyze_spectrum(
    ...
    try:
        ...
        try:
            ...
            # 进行批量光谱分析
            batch_result = await image_service.batch_analyze_spectrum_images(
...
async def visualize_analysis_result(
    ...
    try:
        ...
        # 使用图像服务来格式化分析结果
        visualization_result = await image_service.visualize_analysis_result(analysis_data)
        
        if not visualization_result['success']:
            raise HTTPException(
                status_code=500,
                detail=f"结果可视化失败: {visualization_result.get('error', '未知错误')}"
            )
...
async def health_check():
    ...
    try:
        ...
        status = {
            ...
            "supported_formats": len(image_service.get_supported_image_formats())
        }
